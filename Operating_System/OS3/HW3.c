...
void *insertionFunction(void *parameters)
{
    ...
    for (unsigned i = offset; i < last_ele; i++)
    {
        ...
        pthread_mutex_t *mutex = &mutex_ptr[hash]; 
...
void execute_insertion_function(pthread_t *threads, parameterPass *params, int numOfThreads, hash_table *ht, int *numbers, int numOfElements, pthread_mutex_t *mutex)
{
    ...
    for (unsigned i = 0; i < numOfThreads; i++)
    {
        int result = pthread_create(&threads[i], NULL, insertionFunction, (void *)&params[i]); //This way each individual thread will have its own param values.
        if (result != 0)
        {
...
void execute_bubble_sort(pthread_t *threads, int numOfThreads, hash_table *ht)
{
    ...
    for (unsigned i = 0; i < numOfThreads; i++)
    {
        int result = pthread_create(&threads[i], NULL, execute_bubble_sort_helper_function, (void *)(ht->list[i]));
        if (result != 0)
        {
            printf("Error creating thread %d\n", i);
        }
...
void free_allocated_memory(pthread_t *threads, parameterPass *params, int *numbers, int numberOfBuckets, hash_table *ht, pthread_t *threads2)
{
    free(threads);
    free(threads2);
    free(params);
...
void destroy_mutexes(pthread_mutex_t *mutex_ptr, int numberOfBuckets)
{
...
void print_list_contents(int numberOfBuckets, hash_table *ht)
{
    for (unsigned i = 0; i < numberOfBuckets; i++)
    {
        printf("List %u: \n", i);
...
int main(int argc, char *argv[])
{
    ...
    print_list_contents(numberOfBuckets, ht);

    free_allocated_memory(threads1, params, numbers, numberOfBuckets, ht, threads2);
    destroy_mutexes(mutex_ptr, numberOfBuckets);
    free(mutex_ptr);
    return 0;
}
